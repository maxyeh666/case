...
$kind = system_CleanVars($_REQUEST, 'kind', 'mainShow', 'string');
$kind = "mainShow";
...
switch ($op){
  case "op_delete" :
    ...
    redirect_header("show.php?kind={$kind}", $msg, 2000, "success");
    exit;
  ...
  case "op_update" :
    ...
    redirect_header("show.php", $msg, 2000, "success");
    exit;
...
function op_insert($kind, $sn=""){
  ...
  $_POST['kind'] = db_filter($_POST['kind'], ''); //展示圖
